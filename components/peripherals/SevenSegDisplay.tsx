...
const GLOW = "drop-shadow(0 0 5px #ff8800)";
    ...
    <svg width="30" height="40" viewBox="0 0 30 40" aria-hidden>
      {segs.map((seg, i) => (
        <path
          key={seg}
          d={SEG_PATHS[seg]}
          fill={pattern & (1<<i) ? ON_COLOR : OFF_COLOR}
          style={pattern & (1<<i) ? { filter: GLOW } : undefined}
        />
      ))}
    </svg>
  );
...
/**
 * 5-digit serial-shift-register driven 7-segment display.
 * Driven by GPIO: DATA=P0.19, CLK=P0.20, STROBE=P0.30.
 * The board engine snoops these GPIO edges to update displayBuffer.
 */
export function SevenSegDisplay() {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex gap-2 rounded-md border border-[#2a1800] bg-[#0a0500] px-4 py-3">
        {Array.from({ length: 5 }, (_, i) => (
          <Digit key={i} pattern={displayBuffer[i] ?? 0} />
        ))}
      </div>
      <span className="text-[10px] uppercase tracking-[0.16em] text-muted">
        5-digit 7-seg · DATA P0.19 · CLK P0.20 · STROBE P0.30
      </span>
    </div>
  );
}
